...
func NewIdolModel() tea.Model {
	return &idolModel{
		childMenu:     []string{"角色卡", "支援卡", "回憶卡", "編隊"},
		childCursor:   0,
		siblingCursor: -1,
		isSiblingNav:  false,
	}
...
func (m idolModel) Init() tea.Cmd {
	return nil
}

func (m idolModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	switch msg := msg.(type) {
	case tea.KeyMsg:
		switch msg.String() {
		case "ctrl+c", "q":
